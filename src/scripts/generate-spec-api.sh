...
# Configuration
SPEC_FOLDER="src/api/specifications"
MODEL_FOLDER="src/api/models"
ENDPOINT_FOLDER="src/api/endpoints"
ZIP_FILE="openapi.zip"
...
RAW_FILE_URL="${GITHUB_URL}/raw/${BRANCH}/Specifications/openapi.yaml"

echo "Downloading the OpenAPI spec from: $RAW_FILE_URL..."

# Download the .yaml file
curl -L --output "$SPEC_FOLDER/openapi.yaml" "$RAW_FILE_URL"
...
if [ -f "$SPEC_FOLDER/openapi.yaml" ]; then
    echo "OpenAPI spec downloaded successfully to '$SPEC_FOLDER/openapi.yaml'. ✅"
else
